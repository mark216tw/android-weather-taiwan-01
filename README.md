...
- 顯示紫外線、防曬、日照時數、露點、舒適度、氣壓、分時累積雨量、今日白晝時長、陣風與蒲福風級名稱。
...
- 將完整主頁匯出為 PNG 長圖，並透過 Android 系統分享選單傳送。
...
- [APP 知識庫](docs/APP_KNOWLEDGE_BASE.md)
...
- 有有效目標座標時會選擇全台最近測站；Geocoder 無法提供座標時才退回同鄉鎮或同縣市資料。
- 手動地點的測站距離與日出日落依 Geocoder 近似座標計算，可能與行政區幾何中心略有差異。
