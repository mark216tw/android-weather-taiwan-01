...
本文件整理「台灣天氣」目前的產品行為、資料來源、架構、資料規則與維護注意事項。內容以版本 `1.4.0` 的程式實作為準，適合開發、測試、除錯與專案交接時快速查閱。
...
- 氣象及空氣品質測站時間統一換算為台灣時區，並以 `MM/dd HH:mm` 顯示，不顯示時區與秒數。
- 顯示今日天氣摘要、蒲福風級名稱及今日白晝時長。
- 詳細氣象顯示紫外線分級、防曬建議、日照時數、露點、舒適度、氣壓、分時累積雨量、最大陣風及日出日落時間。
- 顯示 AQI、PM2.5、PM10、O3、CO、SO2、NO2、主要污染物與健康建議。
- 顯示有效中的中央氣象署警特報。
- 未來幾天天氣以全寬卡片垂直排列。
- 顯示快取、過期及部分資料更新失敗狀態。
- 右上角分享按鈕會離屏產生完整首頁 PNG 長圖；詳細氣象與污染物資料在分享圖中全部展開，檔案僅暫存在快取目錄並透過 `FileProvider` 分享。
...
- 日出至日落自動使用淺色模式。
- 日落至隔日日出自動使用深色模式。
- 主要卡的晴天與晴時多雲圖案在日落後改顯示月亮 `🌙`；雷、雨、雪、陰與多雲維持原圖案。
- 每分鐘重新判斷日夜狀態。
...
- 溫度、濕度、露點及氣壓。
- 風速、風向、最大陣風及蒲福風級。
- 目前、1 小時、3 小時及 24 小時累積雨量（依官方可用欄位）。
- 今日白晝時長（由日出至日落計算），並與昨日比較顯示漸增、漸減或持平。
- 測站實際今日日照時數在詳細氣象中顯示。
- 測站名稱、座標及觀測時間。
...
空氣品質卡在淺色模式使用標準 AQI 色彩；深色模式使用相同色相的低亮度、低飽和度色盤，文字則依底色亮度自動選擇黑色或白色。
...
只有時距可明確辨識的 1、3、24 小時累積雨量才參與分級，避免將 `Now.Precipitation` 誤當成 24 小時累積值。

| 等級 | 判斷條件 |
| --- | --- |
| 大雨 | 24 小時達 80 mm，或 1 小時達 40 mm |
| 豪雨 | 24 小時達 200 mm，或 3 小時達 100 mm |
| 大豪雨 | 24 小時達 350 mm，或 3 小時達 200 mm |
| 超大豪雨 | 24 小時達 500 mm |

同時符合多個門檻時只顯示最高等級。缺少可辨識時距時只顯示雨量，不推測分級。

### 蒲福風級與舒適度

- 蒲福 0–7 級依序顯示無風、軟風、輕風、微風、和風、清風、強風、疾風。
- 8–11 級顯示輕度颱風，12–15 級顯示中度颱風，16 級以上顯示強烈颱風。
- 舒適度優先採 CWA 官方描述；缺值時依目前溫度分成很冷、冷、涼爽、稍涼爽、舒適、稍溫暖、悶熱、易中暑及很熱。
